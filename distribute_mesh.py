...
#            Roberto Vidmar replaced pypar with par
#
#
#########################################################
...
def build_local_GA(nodes, triangles, boundaries, tri_map):
    ...
    ##node_map = num.nan * num.empty(int(NGlobal)+1, num.int)
...
def build_local_mesh(submesh, lower_t, upper_t, nproc):
    ...
    ##tri_map = num.nan * num.empty(int(NGlobal) + 1)
...
def send_submesh(submesh, triangles_per_proc, p, verbose=True):
    ...
    par.send(submesh["full_quan"].keys(), p)
...
def rec_submesh_flat(p, verbose=True):
    ...
    par.receive(p, buffer=x,  bypass=True)
    setup_array = x
